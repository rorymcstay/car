...
class Worker:
    ...
    def __init__(self, batch_number, market):
        ...
        self.thread = None
        self.mongoService = MongoService('{mongo_host}:{mongo_port}'.format(**market))
    ...
    def publishObjects(self, results: list):
        ...
        try:
                ...
                if market.get("worker_stream") is not None:
                    ...
                    if market.get("worker_stream").get("single"):
                        self.producer.send(topic="{name}_{type}".format(name=self.market.name, type="car"),
                                           value=str(parser.find(market.get("worker_stream").get("class"))),
            ...
            traceback.print_exc()
            self.webCrawler = WebCrawler(self.market, getOpenPort())
    ...
    def healthCheck(self, exception: Exception = Exception('None')) -> HealthStatus:
        ...
        try:
            ...
            write_log(log=LOG.debug, msg='health_check', **health, collected=str(self.cars_collected))
            self.health = health
            return health
    ...
    def regenerate(self):
        self.port = getOpenPort()
        self.browser = Browser(self.port)
        self.webCrawler = WebCrawler(self.market, port=getOpenPort())
